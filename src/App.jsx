  ...
  async function inviteExternal() {
    ...
    try {
      ...
      const invite = await graphRequest("POST", "/invitations", {
        ...
        invitedUserMessageInfo:  extMsg.trim() ? {
      ...
      showToast(`Invitation sent to ${extEmail.trim()}`, "success");
      setExtEmail(""); setExtName(""); setExtMsg("");
      setModal(null);
    } catch(e) { showToast(`Failed: ${e.message}`, "error"); }
    ...
    <div style={{minHeight:"100vh",background:"#f0f4f8",fontFamily:"'DM Sans',sans-serif"}}>
              ...
              <div style={{background:"linear-gradient(135deg,#1a56db 0%,#0078d4 100%)",borderRadius:18,padding:"22px 26px",marginBottom:20,color:"#fff",boxShadow:"0 8px 32px rgba(26,86,219,.25)"}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
                  <div>
                    ...
                    {selectedG.description && <p style={{opacity:.6,fontSize:12,marginTop:4}}>{selectedG.description}</p>}
                  </div>
      ...
      {modal==="addExternal" && (
        <Modal title="✉ Invite External Guest" onClose={()=>setModal(null)}>
          ...
          <div style={{background:"#fffbeb",borderRadius:9,padding:"12px 14px",marginBottom:18,fontSize:12,color:"#92400e",lineHeight:1.6,border:"1px solid #fde68a"}}>
            ℹ️ Microsoft will send an invitation email. The guest will appear in your group immediately as <strong>Pending</strong> until they accept.
          </div>
          <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
            ...
            <button onClick={inviteExternal} disabled={!extEmail.trim()||submitting}
              ...
              {submitting?<><Spinner size={13}/> Sending…</>:<><span>✉</span> Send Invitation</>}
            </button>
          </div>
        </Modal>
      )}
